...
- **No comment-backed invariants.** If an invariant needs a CI lint to keep a comment honest, express it as a type or package boundary instead (#2495 / #2499 is the pattern). Do not add new lints that validate comments, and do not add tests that do it either — a source-scanning test is the same tax with a different label. Both instances are now gone: `tools/check-router-fields` with the Router facets (G3 #2667) and its Server twin `server_field_access_comments_test.go` (Epic I #2547). The one surviving `// 读写:` note, on `cron.Scheduler`, states a lock discipline rather than a file list, which is an invariant worth writing down.
