...
- 至少一个 IM 平台应用（飞书 / Slack / Discord / 微信）
...
微信通过 iLink Bot API 接入，使用 HTTP 长轮询收消息，**无需公网 IP、域名或端口转发**。

**准备**: 需要两个微信号——一个登录为 bot，另一个用于发消息测试。
...
1. 获取登录二维码:
   ...
   ```
2. 用 bot 微信号扫描 `qrcode_img_content` 中的链接，在手机上确认
3. 轮询登录状态，获取 token:
   ```bash
   curl -s "https://ilinkai.weixin.qq.com/ilink/bot/get_qrcode_status?qrcode=<第1步返回的qrcode值>" \
     -H "iLink-App-ClientVersion: 1"
   # 扫码确认后返回 {"status":"confirmed","bot_token":"xxx","ilink_bot_id":"xxx",...}
   ```
4. 记下 `bot_token`，配置到 `config.yaml` 或环境变量

> **注意**: iLink Bot API 无官方公开文档，协议通过逆向腾讯 OpenClaw 微信插件获得。Token 有效期未知，失效后需重新扫码。
...
所有平台均支持 WebSocket / 长轮询模式，直接运行即可，**无需公网 IP**:

```bash
# 1. 编译
go build -o bin/naozhi ./cmd/naozhi/

# 2. 配置凭据（按需设置对应平台的环境变量）
export FEISHU_APP_ID=your_app_id
export FEISHU_APP_SECRET=your_app_secret
export WEIXIN_BOT_TOKEN=your_bot_token  # 微信扫码获取
...
1. **Token 获取**: 每次需在有微信的设备上扫码，获取的 `bot_token` 写入 `~/.naozhi/env`
2. **Token 失效**: iLink Bot API 的 token 有效期未知，失效后日志会出现认证错误，需重新扫码获取
3. **重新部署**: 更新 token 后只需 `sudo systemctl restart naozhi`，无需重新编译
4. **限制**: 仅支持私聊（1v1），不支持群聊；不支持消息编辑（流式追加）；每条回复需要来自用户的 `context_token`
