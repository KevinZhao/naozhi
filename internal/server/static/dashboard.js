...
// computeHomeStats aggregates allSessionsCache into the three stats surfaced
// on the idle Home panel. Pure function so a contract test can exercise the
// "today" boundary and the cost / prompt summations without driving the DOM.
//
// Scope: the TODO (R110-P1) lists 4 metrics (today active / prompts processed
// / tokens / cost). Three of those ride fields already present on each
// /api/sessions row — today active, total cost, and total prompts (the
// per-session message_count cumulative user-turn counter). Only cumulative
// *token* count still needs a backend event-log scan, so it stays deferred.
//
...
//   totalPrompts — sum of s.message_count (cumulative user turns) across all
//                  cached sessions; same not-gated-by-today rationale as cost
//                  so the running total survives an overnight cron burst.
//
...
function computeHomeStats(items, nowMs) {
  ...
  for (const s of arr) {
    ...
    if (typeof s.message_count === 'number' && isFinite(s.message_count)) totalPrompts += s.message_count;
  }
...
function renderRecentSessionsPanel() {
  ...
  // R110-P1 Home stats strip (Round 147→149): today-active + total cost +
  // total prompts. Rendered above the list so operators see a cumulative
  // signal before scanning the session rows. Cumulative token count still
  // needs a backend event-log scan — omitted here.
  ...
  const statsHtml =
    '<div class="recent-panel-stats" role="group" aria-label="今日概览">' +
      ...
      '<div class="recent-stat">' +
        ...
        '<div class="recent-stat-label">累计 prompt</div>' +
      '</div>' +
      '<div class="recent-stat">' +
