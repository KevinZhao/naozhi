...
func (s *Server) handleDashboard(w http.ResponseWriter, r *http.Request) {
	...
	// script-src is tracked NEEDS-DESIGN, two additive directives close real
	...
	w.Header().Set("Content-Security-Policy", "default-src 'self'; script-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net; connect-src 'self'; style-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net; font-src 'self' https://cdn.jsdelivr.net; img-src 'self' data: blob:; frame-src 'self' blob:; base-uri 'none'; form-action 'self'; frame-ancestors 'none'; require-sri-for script style font")
