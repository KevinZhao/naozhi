package server

import (
	"net/http"
	"net/http/httptest"
	"strings"
	"testing"
)

// TestDashboardJS_RequiresAuth_TokenMode pins R20260527122801-SEC-4
// (#1328) and R249-SEC-10 (#923): in token mode, GET /static/dashboard.js
// MUST return 401 to unauthenticated callers AND the 401 body MUST NOT
// leak any JS source. The previous mux entry served the JS module to
// anyone, leaking the API endpoint list, cron polling cadence, and
// client-side schema to scanners — a free recon surface that the
// dashboard editor has no functional reason to expose pre-login.
func TestDashboardJS_RequiresAuth_TokenMode(t *testing.T) {
	t.Parallel()
	srv := newTestServerWithToken(&mockPlatform{}, "secret")

	req := httptest.NewRequest(http.MethodGet, "/static/dashboard.js", nil)
	w := httptest.NewRecorder()
	srv.mux.ServeHTTP(w, req)

	if w.Code != http.StatusUnauthorized {
		t.Fatalf("/static/dashboard.js unauth GET = %d, want 401 (regression of SEC-4 #1328: scanner can fingerprint deployment via JS body)", w.Code)
	}
	// #923: even on 401, ensure no JS source bytes leak into the body.
	// The body should be the plain "unauthorized" / empty 401 response,
	// not a slice of the embedded asset.
	body := w.Body.String()
	for _, leak := range []string{"PLATFORM_ORIGINS", "function ", "/api/sessions"} {
		if strings.Contains(body, leak) {
			t.Errorf("#923 regression: 401 body leaks JS source token %q (would let scanner fingerprint deployment)", leak)
		}
	}
}

// TestAgentViewJS_RequiresAuth_TokenMode is the companion guard for the
// other JS module that ships dashboard internals. Same regression class
// (#1328 / #923).
func TestAgentViewJS_RequiresAuth_TokenMode(t *testing.T) {
	t.Parallel()
	srv := newTestServerWithToken(&mockPlatform{}, "secret")

	req := httptest.NewRequest(http.MethodGet, "/static/agent_view.js", nil)
	w := httptest.NewRecorder()
	srv.mux.ServeHTTP(w, req)

	if w.Code != http.StatusUnauthorized {
		t.Fatalf("/static/agent_view.js unauth GET = %d, want 401 (regression of SEC-4 #1328)", w.Code)
	}
	// #923: 401 body must not leak source bytes from agent_view.js.
	body := w.Body.String()
	for _, leak := range []string{"function ", "agentView", "subagent"} {
		if strings.Contains(body, leak) {
			t.Errorf("#923 regression: 401 body leaks agent_view.js source token %q", leak)
		}
	}
}

// TestDashboardJS_NoTokenModeStillServes pins the no-token-mode escape
// valve: requireAuth is a pass-through when dashboardToken == "" (single-
// user / loopback deployments), so the JS module still loads on a fresh
// request without any auth header. This is the legacy contract the SEC-4
// fix must NOT break — a no-token operator has no login page in front of
// the dashboard, and the JS file load is part of the bootstrap.
func TestDashboardJS_NoTokenModeStillServes(t *testing.T) {
	t.Parallel()
	srv := newTestServer(&mockPlatform{}) // no token -> no-token mode

	req := httptest.NewRequest(http.MethodGet, "/static/dashboard.js", nil)
	w := httptest.NewRecorder()
	srv.mux.ServeHTTP(w, req)

	if w.Code != http.StatusOK {
		t.Fatalf("/static/dashboard.js no-token GET = %d, want 200; body=%q", w.Code, w.Body.String())
	}
}

// TestAgentViewJS_NoTokenModeStillServes mirrors the no-token contract
// for the agent-team-ui module so the same escape valve is locked.
func TestAgentViewJS_NoTokenModeStillServes(t *testing.T) {
	t.Parallel()
	srv := newTestServer(&mockPlatform{})

	req := httptest.NewRequest(http.MethodGet, "/static/agent_view.js", nil)
	w := httptest.NewRecorder()
	srv.mux.ServeHTTP(w, req)

	if w.Code != http.StatusOK {
		t.Fatalf("/static/agent_view.js no-token GET = %d, want 200; body=%q", w.Code, w.Body.String())
	}
}

// TestAssetBrowserJS_RequiresAuth_TokenMode guards the cc-asset-browser module
// against the same SEC-4 (#1328) fingerprinting regression.
func TestAssetBrowserJS_RequiresAuth_TokenMode(t *testing.T) {
	t.Parallel()
	srv := newTestServerWithToken(&mockPlatform{}, "secret")

	req := httptest.NewRequest(http.MethodGet, "/static/asset_browser.js", nil)
	w := httptest.NewRecorder()
	srv.mux.ServeHTTP(w, req)

	if w.Code != http.StatusUnauthorized {
		t.Fatalf("/static/asset_browser.js unauth GET = %d, want 401 (SEC-4 #1328)", w.Code)
	}
	body := w.Body.String()
	for _, leak := range []string{"openAssetBrowser", "ab-overlay", "function "} {
		if strings.Contains(body, leak) {
			t.Errorf("#923 regression: 401 body leaks asset_browser.js source token %q", leak)
		}
	}
}

// TestAssetBrowserJS_NoTokenModeStillServes locks the no-token escape valve.
func TestAssetBrowserJS_NoTokenModeStillServes(t *testing.T) {
	t.Parallel()
	srv := newTestServer(&mockPlatform{})

	req := httptest.NewRequest(http.MethodGet, "/static/asset_browser.js", nil)
	w := httptest.NewRecorder()
	srv.mux.ServeHTTP(w, req)

	if w.Code != http.StatusOK {
		t.Fatalf("/static/asset_browser.js no-token GET = %d, want 200; body=%q", w.Code, w.Body.String())
	}
}

// TestNzUtilJS_RequiresAuth_TokenMode guards the shared utility layer
// (nz_util.js, RFC dashboard-cron-view-extraction PR-0a) against the same
// SEC-4 (#1328) fingerprinting regression as the other static JS modules.
func TestNzUtilJS_RequiresAuth_TokenMode(t *testing.T) {
	t.Parallel()
	srv := newTestServerWithToken(&mockPlatform{}, "secret")

	req := httptest.NewRequest(http.MethodGet, "/static/nz_util.js", nil)
	w := httptest.NewRecorder()
	srv.mux.ServeHTTP(w, req)

	if w.Code != http.StatusUnauthorized {
		t.Fatalf("/static/nz_util.js unauth GET = %d, want 401 (SEC-4 #1328)", w.Code)
	}
	body := w.Body.String()
	for _, leak := range []string{"function ", "fetchJSON", "escJs"} {
		if strings.Contains(body, leak) {
			t.Errorf("#923 regression: 401 body leaks nz_util.js source token %q", leak)
		}
	}
}

// TestNzUtilJS_NoTokenModeStillServes locks the no-token escape valve.
func TestNzUtilJS_NoTokenModeStillServes(t *testing.T) {
	t.Parallel()
	srv := newTestServer(&mockPlatform{})

	req := httptest.NewRequest(http.MethodGet, "/static/nz_util.js", nil)
	w := httptest.NewRecorder()
	srv.mux.ServeHTTP(w, req)

	if w.Code != http.StatusOK {
		t.Fatalf("/static/nz_util.js no-token GET = %d, want 200; body=%q", w.Code, w.Body.String())
	}
}

// TestCronViewJS_RequiresAuth_TokenMode guards the cron view module
// (cron_view.js, RFC dashboard-cron-view-extraction PR-1) against the same
// SEC-4 (#1328) fingerprinting regression as the other static JS modules.
func TestCronViewJS_RequiresAuth_TokenMode(t *testing.T) {
	t.Parallel()
	srv := newTestServerWithToken(&mockPlatform{}, "secret")

	req := httptest.NewRequest(http.MethodGet, "/static/cron_view.js", nil)
	w := httptest.NewRecorder()
	srv.mux.ServeHTTP(w, req)

	if w.Code != http.StatusUnauthorized {
		t.Fatalf("/static/cron_view.js unauth GET = %d, want 401 (SEC-4 #1328)", w.Code)
	}
	body := w.Body.String()
	for _, leak := range []string{"function ", "openCronPanel", "cronJobs"} {
		if strings.Contains(body, leak) {
			t.Errorf("#923 regression: 401 body leaks cron_view.js source token %q", leak)
		}
	}
}

// TestCronScheduleJS_RequiresAuth_TokenMode: same SEC-4 gate for the
// schedule/picker region split out of cron_view.js (#2715 D4 follow-up).
func TestCronScheduleJS_RequiresAuth_TokenMode(t *testing.T) {
	t.Parallel()
	srv := newTestServerWithToken(&mockPlatform{}, "secret")

	req := httptest.NewRequest(http.MethodGet, "/static/cron_schedule.js", nil)
	w := httptest.NewRecorder()
	srv.mux.ServeHTTP(w, req)

	if w.Code != http.StatusUnauthorized {
		t.Fatalf("/static/cron_schedule.js unauth GET = %d, want 401 (SEC-4 #1328)", w.Code)
	}
	body := w.Body.String()
	for _, leak := range []string{"function ", "humanizeCron", "buildFreqSchedule"} {
		if strings.Contains(body, leak) {
			t.Errorf("#923 regression: 401 body leaks cron_schedule.js source token %q", leak)
		}
	}
}

// TestCronTimelineJS_RequiresAuth_TokenMode: same SEC-4 gate for the
// timeline region split out of cron_view.js (#2715 D4 follow-up).
func TestCronTimelineJS_RequiresAuth_TokenMode(t *testing.T) {
	t.Parallel()
	srv := newTestServerWithToken(&mockPlatform{}, "secret")

	req := httptest.NewRequest(http.MethodGet, "/static/cron_timeline.js", nil)
	w := httptest.NewRecorder()
	srv.mux.ServeHTTP(w, req)

	if w.Code != http.StatusUnauthorized {
		t.Fatalf("/static/cron_timeline.js unauth GET = %d, want 401 (SEC-4 #1328)", w.Code)
	}
	body := w.Body.String()
	for _, leak := range []string{"function ", "cronTimelineExpand", "cronExpandedRunId"} {
		if strings.Contains(body, leak) {
			t.Errorf("#923 regression: 401 body leaks cron_timeline.js source token %q", leak)
		}
	}
}

// TestCronDrawerJS_RequiresAuth_TokenMode: same SEC-4 gate for the drawer
// region split out of cron_view.js (#2715 D4 follow-up).
func TestCronDrawerJS_RequiresAuth_TokenMode(t *testing.T) {
	t.Parallel()
	srv := newTestServerWithToken(&mockPlatform{}, "secret")

	req := httptest.NewRequest(http.MethodGet, "/static/cron_drawer.js", nil)
	w := httptest.NewRecorder()
	srv.mux.ServeHTTP(w, req)

	if w.Code != http.StatusUnauthorized {
		t.Fatalf("/static/cron_drawer.js unauth GET = %d, want 401 (SEC-4 #1328)", w.Code)
	}
	body := w.Body.String()
	for _, leak := range []string{"function ", "openCronDetail", "cronDrawerHtml"} {
		if strings.Contains(body, leak) {
			t.Errorf("#923 regression: 401 body leaks cron_drawer.js source token %q", leak)
		}
	}
}

// TestCronTriggerJS_RequiresAuth_TokenMode: same SEC-4 gate for the trigger
// region split out of cron_view.js (#2715 D4 follow-up).
func TestCronTriggerJS_RequiresAuth_TokenMode(t *testing.T) {
	t.Parallel()
	srv := newTestServerWithToken(&mockPlatform{}, "secret")

	req := httptest.NewRequest(http.MethodGet, "/static/cron_trigger.js", nil)
	w := httptest.NewRecorder()
	srv.mux.ServeHTTP(w, req)

	if w.Code != http.StatusUnauthorized {
		t.Fatalf("/static/cron_trigger.js unauth GET = %d, want 401 (SEC-4 #1328)", w.Code)
	}
	body := w.Body.String()
	for _, leak := range []string{"function ", "cronTriggerNow", "cronJustTriggered"} {
		if strings.Contains(body, leak) {
			t.Errorf("#923 regression: 401 body leaks cron_trigger.js source token %q", leak)
		}
	}
}

// TestCronViewJS_NoTokenModeStillServes locks the no-token escape valve.
func TestCronViewJS_NoTokenModeStillServes(t *testing.T) {
	t.Parallel()
	srv := newTestServer(&mockPlatform{})

	req := httptest.NewRequest(http.MethodGet, "/static/cron_view.js", nil)
	w := httptest.NewRecorder()
	srv.mux.ServeHTTP(w, req)

	if w.Code != http.StatusOK {
		t.Fatalf("/static/cron_view.js no-token GET = %d, want 200; body=%q", w.Code, w.Body.String())
	}
}
