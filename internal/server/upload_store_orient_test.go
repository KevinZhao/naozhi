...
func TestUploadStorePeek_DoesNotConsume(t *testing.T) {
	s := newUploadStore()
	img := cli.Attachment{Data: []byte("original"), MimeType: "image/jpeg"}
...
func TestUploadStorePeek_OwnerIsolation(t *testing.T) {
	s := newUploadStore()
	id, _ := s.Put("alice", cli.Attachment{Data: []byte("secret"), MimeType: "image/jpeg"})
...
func TestUploadStorePeek_ReturnsCopy(t *testing.T) {
	s := newUploadStore()
	id, _ := s.Put("alice", cli.Attachment{Data: []byte("abcd"), MimeType: "image/jpeg"})
	got := s.Peek(id, "alice")
	if got == nil {
...
func TestUploadStoreReplace_UpdatesBytes(t *testing.T) {
	s := newUploadStore()
	id, _ := s.Put("alice", cli.Attachment{Data: []byte("original"), MimeType: "image/jpeg"})

	ok := s.Replace(id, "alice", cli.Attachment{Data: []byte("rotated!!"), MimeType: "image/jpeg"})
	if !ok {
...
func TestUploadStoreReplace_OwnerIsolation(t *testing.T) {
	...
	if s.Replace(id, "bob", cli.Attachment{Data: []byte("evil"), MimeType: "image/jpeg"}) {
		t.Error("Replace must reject a non-owner")
	}
	if s.Replace("nonexistent", "alice", cli.Attachment{Data: []byte("x"), MimeType: "image/jpeg"}) {
...
func TestUploadStoreReplace_ByteAccounting(t *testing.T) {
	s := newUploadStore()
	id, _ := s.Put("alice", cli.Attachment{Data: make([]byte, 1000), MimeType: "image/jpeg"})

	// Shrink to 100 bytes.
	if !s.Replace(id, "alice", cli.Attachment{Data: make([]byte, 100), MimeType: "image/jpeg"}) {
	...
	// Grow to 500 bytes.
	if !s.Replace(id, "alice", cli.Attachment{Data: make([]byte, 500), MimeType: "image/jpeg"}) {
...
func TestUploadStoreReplace_AfterTakeFailsSafe(t *testing.T) {
	...
	if s.Replace(id, "alice", cli.Attachment{Data: []byte("rotated"), MimeType: "image/jpeg"}) {
...
func TestUploadStoreReplace_RejectsOverQuota(t *testing.T) {
	...
	huge := cli.Attachment{Data: make([]byte, maxUploadBytesPerOwner+1), MimeType: "image/jpeg"}
