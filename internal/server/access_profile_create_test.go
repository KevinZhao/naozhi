...
func newCreateProfileServer(t *testing.T) (*Server, *handlerSet, string, string) {
	t.Helper()
	dir := t.TempDir()
...
func postCreateProfile(t *testing.T, hs *handlerSet, body string) *httptest.ResponseRecorder {
	t.Helper()
	req := httptest.NewRequest(http.MethodPost, "/api/access-profiles", strings.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()
	hs.accessProfilesH.HandleCreate(w, req)
	return w
}

func TestCreateAccessProfile_BedrockNoSecret(t *testing.T) {
	srv, hs, cfgPath, _ := newCreateProfileServer(t)
...
func TestCreateAccessProfile_WithSecretFile(t *testing.T) {
	_, hs, cfgPath, secretsDir := newCreateProfileServer(t)
...
func TestCreateAccessProfile_Rejections(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			_, hs, _, _ := newCreateProfileServer(t)
			w := postCreateProfile(t, hs, tc.body)
			if w.Code != tc.want {
...
func TestCreateAccessProfile_Duplicate(t *testing.T) {
	...
	if w := postCreateProfile(t, hs, body); w.Code != http.StatusOK {
		t.Fatalf("first create failed: %d %s", w.Code, w.Body.String())
	}
	if w := postCreateProfile(t, hs, body); w.Code != http.StatusConflict {
...
func TestCreateAccessProfile_DisabledWithoutConfigPath(t *testing.T) {
	...
	w := postCreateProfile(t, hs, `{"id":"x","env":{}}`)
...
func TestCreateAccessProfile_OrphanSecretCleanedOnFailure(t *testing.T) {
	_, hs, _, secretsDir := newCreateProfileServer(t)
