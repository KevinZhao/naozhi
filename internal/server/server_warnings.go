// Server.Start 启动期安全 self-check：plaintext / no-token 警告决策矩阵与文案常量。
package server
...
// on a non-loopback plaintext HTTP bind with no trusted proxy in front.
// Named so tests can assert the exact journal text.
...
//	reverse server,    public,   trustedProxy=false    → WARN
...
//	token "",   public,   trustedProxy=false → WARN
