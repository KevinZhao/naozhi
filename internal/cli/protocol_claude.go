...
func (p *ClaudeProtocol) BuildArgs(opts SpawnOptions) []string {
	...
	// codes — 429 throttles vs 5xx — are observable) to that file. `--debug-file`
	// implicitly enables debug; no category filter is passed because the filter
	// only scopes the CLI's terminal/stderr stream, never the file (verified:
	// file output is byte-identical with or without `--debug api`). The path is
	// router-generated under the data root, but we still reject a leading '-' so
