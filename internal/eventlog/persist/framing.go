...
// WriteRecordRaw is the production-side framed write — it takes an
// already-marshalled record body and wraps it in the length-prefix
// framing (<decimal-length>\n<body>\n).
//
// A single Write() is NOT what this guarantees: writeFramedBody emits
// four small Writes that the buffered writer (Persister wraps logFile
// in *bufio.Writer) coalesces. The framing's job is to let readers
// detect torn writes; the per-key single-writer invariant + bufio is
// what serialises producers.
//
...
// bytes and the trailing newlines) so the Persister can update its
// per-file byte counter for rotate threshold detection.
