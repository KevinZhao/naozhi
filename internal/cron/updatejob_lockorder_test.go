...
// s.registerJob while holding s.mu, because both send on unbuffered
// robfig/cron channels drained by the run loop, and a tick callback reaches
// executeJobIDIfLive → s.mu.RLock, so holding s.mu across them inverts the
...
// the order s.mu → c.runningMu is real. An inversion needs the opposite order
...
// nothing ever reaches s.mu with c.runningMu held, and the cycle cannot close.
...
//
//   - line 552 already calls s.registerJob under s.mu.Lock (the entryID
//     write-back block), which is exactly what the anchor's message forbids,
//     and the anchor was green;
//   - moving s.cron.Remove into that same s.mu.Lock — the most direct possible
...
// version-independent: it puts UpdateJob's cron ops in flight against s.mu
...
func TestUpdateJob_ScheduleChange_SurvivesConcurrentReaders(t *testing.T) {
	...
	case <-time.After(10 * time.Second):
		t.Fatal("timed out after 10s: UpdateJob's schedule-change branch wedged " +
			"against concurrent s.mu readers")
	}
