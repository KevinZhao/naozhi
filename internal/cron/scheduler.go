...
// MUST mutate it only through the WithStopBudget helper (defined in
// stop_budget_test.go) so the var swap is paired with a t.Cleanup
// restore — direct writes from t.Parallel tests would race a concurrent
// Stop on another Scheduler instance with real wall-clock timeouts.
