...
//
// EnsureStub 的 godoc 声明 caller 必须 NOT hold s.tbl.mu — 实现内部走 RLock。
// 没有静态防护时，未来 caller 可能在持 s.tbl.mu.Lock 的代码路径下调用本方法，
// RWMutex 不可重入会立刻死锁；任何把 EnsureStub 改成 s.tbl.mu.Lock 的"修复"
// 也会让现有 callers (handleSubscribe 等) 在持 s.tbl.mu.RLock 时调用而死锁。
//
// 该测试用并发模型把契约转成可机器验证的不变量：
//
//  1. 从主 goroutine 持有 s.tbl.mu.RLock；EnsureStub 必须在并发协程能立刻完成
//     —— 证明它使用 RLock-compatible 路径，没有升级到 Lock。
//  2. 主 goroutine 持有 s.tbl.mu.Lock 时，EnsureStub 在并发协程必然阻塞
//     —— 证明它确实拿 RLock（而非完全 lock-free 读取脏 jobs map）。
//
// 任何回归把 EnsureStub 改成 s.tbl.mu.Lock 都会让步骤 1 死锁；改成 lock-free
// 读 s.tbl.jobs 又会让步骤 2 假阳性通过——双向夹逼把"必须使用 RLock"刻在测试里。
