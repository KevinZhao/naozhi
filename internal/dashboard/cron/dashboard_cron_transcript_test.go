...
// Note: the path-containment helper that backs the case-insensitive fallback
// for the path-escape gate (R238-SEC-6) was hoisted to
// osutil.PathContainedInRoot; its unit coverage now lives in
// internal/osutil/pathroot_test.go (TestPathContainedInRoot_*). The
// handler-level happy-path test above still covers the integration here.
