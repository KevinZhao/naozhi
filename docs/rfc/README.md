...
| [codex-backend.md](codex-backend.md) | Ready for implementation v2（源码核对；运行时实测待跑） | 2026-06-18 | OpenAI Codex CLI 作为第三个 backend：走 `codex app-server` 长连接（**非真 JSON-RPC 2.0**，不带 `jsonrpc` 字段；**非** `codex proto` / `exec --json` / `mcp-server`），新增 `protocol_codex.go` + `profile_codex.go` + `codexjsonl` 历史 source。协议 wire 形状已基于 openai/codex@main `c73296a0` 逐方法核对并 pin（含 turn/start 立即返回、token 走独立 `thread/tokenUsage/updated`、approval decision 枚举），naozhi 接线点已对 master `cd7334ec` 工作树核对；运行时实测 V1-V12 仍待在有 codex 的环境补跑 |
