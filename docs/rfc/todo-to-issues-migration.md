...
cron-buildfix-attempted           # PR Merge cron v6 已对该 PR 尝试过 build-fix 自救
cron-rebase-attempted             # PR Merge cron v6 已对该 PR 尝试过 auto-rebase 自救
```
...
> **v6 (2026-05-26)**: PR Merge cron 在落 retry/comment 之前先尝试一次自救：
> - **DIRTY → auto-rebase**: 拉 PR 到独立 worktree → `git rebase origin/master` →
>   reasoning 解冲突 → 本地 `GOOS=linux/darwin/windows go build + vet + race test`
>   全绿 → `git push --force-with-lease`。冲突文件 > 3 / 校验失败 → fall back。
> - **UNSTABLE/BLOCKED → build-fix**: 拉 PR 日志，pattern gate 限定 build error
>   （`undefined:` / `imported and not used` / `cannot use .* as` 等），禁止处理
>   `--- FAIL:` / `race detected` / `panic`；改动 ≤ 1 文件 / ≤ 30 行，仅允许
>   build-tag / shim / import / 类型断言；同上跨平台 build + race test 全绿才推。
> - 每 PR 各项自救 ≤ 1 次，gate 看 `cron-buildfix-attempted` / `cron-rebase-attempted`
>   label，避免循环。
...
| Hourly PR Review & Merge | `0 * * * *` | v5 分流: cron PR 必须带 `cron-reviewed-pass\|changes` label 才合（review gate）；人写 PR **默认合**（CLEAN + 全绿），命中 `(breaking-local)` / Draft / `human-review-required` / `do-not-merge` 才跳过；merge 后 `gh pr view --json closingIssuesReferences` verify issue 真 closed，漏关补 close。**v6 自救**: DIRTY 先 auto-rebase 一次（≤3 文件冲突），UNSTABLE/BLOCKED 先 build-fix 一次（仅 build error pattern）；都做本地 GOOS=linux/darwin/windows build + vet + race test 校验后再 `--force-with-lease` push |
...
**cron PR v6 自救分支**（在 4 之前命中 DIRTY / UNSTABLE 时介入）：

```
4a. DIRTY → auto-rebase 子流程
   - 独立 worktree co origin/$HEAD; git fetch master
   - 预检冲突 ≤ 3 文件，否则 abort
   - git rebase origin/master，reasoning 解冲突（master is truth 优先）
   - GOOS=linux/darwin/windows go build + vet + race test 全绿
   - git push --force-with-lease + label cron-rebase-attempted
4b. UNSTABLE/BLOCKED → build-fix 子流程
   - 独立 worktree, gh run view --log-failed 拉 build error
   - pattern gate 限定（undefined: / imported and not used / cannot use as）
   - 禁止处理 test failure / race / panic
   - 改动 ≤ 1 文件 / ≤ 30 行，仅 build-tag / shim / import / 类型断言
   - 同上跨平台 build + race test 校验后 push + label cron-buildfix-attempted
4c. 自救成功 → 等 CI 重跑（下一轮 cron tick 进 4 走 CLEAN 合）
4d. 自救失败 → fall back 到原 retry / comment 路径
```
...
| v6 auto-rebase 误解冲突 | v6 (2026-05-26) | 仅 cron PR；冲突 ≤ 3 文件；reasoning 不确定立即 abort；本地跨平台 build + race test 全绿才推；每 PR ≤ 1 次（label gate） |
| v6 build-fix 改业务逻辑 | v6 (2026-05-26) | pattern gate 限 build error，禁 test/race/panic；改动 ≤ 1 文件 / ≤ 30 行；仅允许 build-tag/shim/import/类型断言；同上跨平台校验 |
| `--force-with-lease` 误推 | v6 (2026-05-26) | 仅推 cron 自己开的 `cron/*` 分支（人写 PR 不动）；力度上比 `--force` 安全（远端被人改过会拒推） |
...
- ✅ docs/TODO_ARCHIVE.md / TODO_CHANGELOG.md / TODO-changelog.md → 初期留作只读 reference + banner 标注，2026-05-30 删除（内容仍在 git 历史，可 `git log -S` 反查）
...
- R-anchor 仍可在 git 历史里反查（`git log -S 'R-XXX'`，含已删的 `docs/TODO*.md` 及归档）
...
8. **prompt 里"不阻挡自救"的强调一句没写，cron 就把整个 PR 跳过** (2026-05-26): v6 第一次 trigger 后 cron 看到「24h 已 comment 过」就把 #1228 整体跳过，没进 v6 子流程；prompt 里 case `DIRTY` 分支虽然先调 `try_rebase`，但 LLM 把"24h-commented gate"当成更高优先级的 PR 级 short-circuit。补一句"24h gate 只阻挡重复 comment，不阻挡 v6 自救"立刻生效。教训：prompt 改新功能时要明确写**它和已有 gate 的优先级关系**，不能依赖 LLM 自己推出来
9. **prompt 8KB cap 紧** (2026-05-26): v6 写到 15KB → 必须压到 7945B 才过 `MaxPromptBytes = 8 * 1024`。压缩主要靠：把详细 bash 步骤改成精炼步骤列表（让 CC 自己写代码）+ 去掉重复描述。原 prompt 后期会越加越大，应该单独评估：要么提 cap，要么把流程拆成多个 cron + skill
...
- PR #1228 (v6 auto-rebase + build-fix 首次实测：DIRTY 解冲突 + windows shim 修复，全自动收掉 cron PR)
- `.claude/skills/triage-findings/SKILL.md`
