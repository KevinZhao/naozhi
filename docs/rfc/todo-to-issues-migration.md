...
cron-mergeable                    # 人写 PR 显式 opt-in cron 自动合
```
...
| Daily PR Review & Merge | `0 * * * *` | 新增「分支识别」: `cron/*` 前缀自动合，其他必须 `cron-mergeable` label 才合；merge 后用 `gh pr view --json closingIssuesReferences` 验证 issue 真 closed，漏关补 close |
...
3. PR Merge cron 下次 tick (≤ 1h)
4. 识别 cron/* 分支 + CLEAN + 全绿 → 自动 squash merge + delete branch
5. GitHub Closes #N #M ... 自动 close 关联 issue
6. PR Merge cron verify-pass 兜底
```

人写 PR 不在此 path，必须人审 + 加 `cron-mergeable` label opt-in。
...
| cron auto-merge 把人写 PR 误合 | 已有 v3 prompt | 分支非 `cron/*` 必须 `cron-mergeable` label opt-in |
