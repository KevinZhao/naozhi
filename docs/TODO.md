...
- [x] **R227-CR-8 — `TodosDetailJSON` 二次 marshal（P2）**: ParseTodos Unmarshal 后又 Marshal。方案：直接抽取 block.Input 的 todos 字段原始字节。重申 R226-PERF-8。 — 已关闭（与 R226-PERF-8 / R228-PERF-2 同根因：`internal/cli/todo.go::ParseTodosWithRaw` 已返回 `(todos, rawTodos, ok)`，`process_event_format.go` TodoWrite 分支直接 `entry.Detail = string(rawTodos)` 不再走 `TodosDetailJSON` 二次 marshal。R226-PERF-8 落地于 PR #166，R228-PERF-2 同步关闭归档于 PR #183），本批 PR #PENDING
