...
- [~] **R62-GO-3 — `ResetAndRecreate` 释放 + 重取 `r.mu` 窗口对 `spawnSession` opts 的竞态（MED）**: `router.go:1532-1538` 删 session 后释放 mu 调用 `proc.Close()`，再 re-Lock 调 spawnSession。此窗口内若并发 `GetOrCreate` 抢先 spawn 同 key session，其 opts 会覆盖 ResetAndRecreate 调用方的 Backend 选择，而调用方以为拿到了自己 opts 下的 session。 — 归档 2026-05-23 (superseded)：`internal/session/router_lifecycle.go:1419-1437` 已 NOTE 说明 mitigation（CAS gate + 调用方契约）；语义已锁定，本条不再独立跟踪
...
- [~] **R37-REL1 — `MessageQueue.TryAcquire` + `Release` 不会触发 drain**: Dashboard/WS 路径用 Guard 接口，但若同期 IM 入口 Enqueue 了消息（enqueued=true），Release 不会触发 DoneOrDrain，消息永久搁浅直到下次 Enqueue 再成为 owner。属于 Guard/Queue 混用的根本限制。 — 归档 2026-05-23 (superseded)：`internal/dispatch/msgqueue.go:84-93` godoc 已记载此为 Guard/Queue 混用的根本约束；不修复属于设计取舍
...
- [~] **R233-PERF-4 — TranscriptReader.readLocked 每次 Tail open+ReadAll+close（P2）**: 50 tailer × 5/s = 250 syscall/s。方案：持久化 *os.File，每 Tail 只 ReadAt(offset)，inode 变更时重开。Breaking：否。 — 归档 2026-05-23 (superseded by R230B-PERF-5)：`internal/cli/subagent_transcript.go:240-245` 已加 archive anchor，与 R230B-PERF-5 同根因；R232-PERF-3 readBuf 复用已消除 ReadAll 增长，剩余持久化 fd 由 R230B-PERF-5 主条目跟踪
...
- [x] **R233-PERF-7 — agentTailer 200ms ticker buffered 超 500 时 append([]EventEntry(nil), …) 复制 500 条（P3）**: 每 poll 150KB 内存 copy。方案：ring buffer。Breaking：否。 — 已修复 2026-05-23：`internal/server/agent_tailer.go:360-381` 改为 in-place `copy(t.buffered, t.buffered[over:])` + 尾部 zero-out + slice re-slice，避免每 poll 重新分配 backing array；语义等价 ring buffer FIFO 截断，本批 PR
...
- [~] **R231-ARCH-10 — backend.Profile 注册表承诺与 wrapper.go 硬编码 switch 落差（P2）**: `cli/wrapper.go` `backendDisplayName` / `detectCLI` 仍硬编码 switch on "kiro" / "claude"，DESIGN.md L280 承诺已部分兑现但未到位。方案：通过 `backend.LookupProfile(id).DisplayName/.DefaultBinary` 获取，删除硬编码 switch；测试加 contract 锁。 — 评估关闭（2026-05-23 R234 复核）：`internal/cli/wrapper.go:detectCLI` godoc 锚点已加 R231-ARCH-10 / R225-CR-5 cross-ref，记录 cli↔backend 单向 import cycle 约束（backend 已 import cli 以拿 Protocol 接口；反向 cli→backend 形环）。真修复需先把 Protocol 提到独立子包或拆 profileonly 子包；当前两条 switch ≤2 行 × 2 处，第三个 backend（gemini-cli）落地时统一改造收益更高。
...
- [~] **R230-ARCH-7 — 错误→用户消息映射有 3 处偏序（P2）（部分修复 2026-05-23 / godoc 锚点）**: dispatch.sendAndReply GetOrCreate 错误 switch 加 R230-ARCH-7 godoc 段：明确说明 ErrMaxProcs / ErrMaxExemptSessions / ErrNoCLIWrapper / context.Canceled 在 usermsg.ForSendError 已注册，但 dispatch 内联保留两个 *delivery-path* 不可外移的发散：(1) GetOrCreate 默认文案"会话创建失败"vs usermsg 通用"处理失败"；(2) context.Canceled 需要替换 replyCtx 为 Background（R188-CONC-M1）—— usermsg 返回 string 触不到 caller 的 ctx 变量。同时 godoc 加新加 sentinel 流程"先 usermsg 注册再判 dispatch 是否需要 override"。本批 PR。剩余 apierr.localizeAPIError 第四套合并跟踪 R226-ARCH-12。
...
- [x] **R226-PERF-10 — `process_shim_io.shimWriter.Write` fast path `string(data[:len-1])` 拷贝（P3，封 R71-PERF-H1）**: shimClientMsg.Line 改 `json.RawMessage`。 → R234 godoc anchor: shimWriter.Write 顶部加 R71/R226 cross-ref，说明 `shimClientMsg.Line` 改 `json.RawMessage` 需配套 shim wire-protocol revision + `returnShimSendEnc` slice-zero；当前迁移代价 > 收益（5-50 events/s × ~200B alloc），改完跨 shim peer 兼容才推进。
...
- [x] **R224-GO-6 — `shim/server.go SetReadDeadline` 错误 nolint 静默吞（P2）**: `:654, :680` SetReadDeadline 失败 nolint:errcheck 直接吞；conn 已关闭时后续 ReadBytes 无 deadline 阻塞 goroutine 泄漏；deadline 清除失败时 post-auth 读立即 timeout 踢掉合法客户端。方案：失败时显式关闭 conn 并 return。 — 已修复（2026-05-23 复核 internal/shim/server.go:701/732 两处 SetReadDeadline 已捕获 err 并在失败时 return，由 defer conn.Close() 兜底关闭；godoc 锚点同步标注 R224-GO-6），本批 PR
- [x] **R224-GO-7 — `shim/server.go writer goroutine` 内层 `w.Write(more)` 错误 nolint 吞（P2）**: `:785` 写失败后 `:790` 仍调 `flushWithDeadline()`，可能将损坏的 buf 状态 flush 出去。方案：内层 write 失败立即 return。 — 已修复（2026-05-23 复核 internal/shim/server.go:828/840 内/外层 w.Write 失败均直接 return，不再走 flushWithDeadline；godoc 锚点同步标注 R224-GO-7），本批 PR
...
- [x] **R222-CR-4 — `cron.executeOpt` 247 行（P2）**: 抽 recordAndBroadcastRun。涉及：`internal/cron/scheduler.go:1677`。【2026-05-23 R222-CR-4 修复】抽 `recordSuccess(recordSuccessArgs)` 把 success tail（setSessionID/slow-log/finishRun/deliverNotice ~30 行）从 executeOpt 移出，executeOpt 末尾退化为单结构体调用；新 helper 接受扁平化的 SessionID/Text 字段避免 cron 包再 import cli。
...
- [x] **R220-PERF-3 — `EventLog.EntriesSince` 初始 catch-up 在 RLock 下复制 500 entry × 512B（P2）**: 反向扫描+复制全在 l.mu RLock 内，subscriber 初始订阅时阻塞 Append 一段时间。方案：先 snapshot ring 索引（head/count），release RLock，再在临时 slice 内拷贝。涉及：`internal/cli/eventlog.go:869`。 → R234 godoc anchor: 当前 single-pass + lazy `make(initialCap=min(k,16))` 已把稳态 k=1-5 与 catch-up max-ring-size 两条路径都收敛到 RLock 内不可避免的一次拷贝；snapshot-then-unlock 方案要么需第二次拷贝抵消优化、要么需 unsafe 别名破坏 head 旋转下的正确性，保留现状。
