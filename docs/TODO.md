...
- [ ] **R248-TEST-4 — shutdown_lock_order_test 漏扫 6 个 wshub_*.go split 文件（P2）** [REFACTOR]: `internal/server/shutdown_lock_order_test.go:40` PR #327 split 后实际涉锁代码搬到 wshub_subscribe.go / wshub_eventpush.go，invariant A 的 `subMu.Lock(` 扫描永远空跑（token 从未在 wshub*.go 出现）。方案：把 invariant A 的扫描扩到 wshub.go + 全部 6 个 wshub_*.go 文件，真正的 SubscribeEvents/unsub 调用现在在 wshub_subscribe.go:197 和 wshub_eventpush.go:204。
