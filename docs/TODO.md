...
- [x] **R234-GO-5 — sysession.Manager.Stop wg.Wait goroutine 无 WG 跟踪（P2）**：osExit(2) 兜底当前安全（进程死亡），但若测试替换 osExit 为 panic-recovery，goroutine 永久阻塞。建议加注释 `// goroutine intentionally abandoned — osExit terminates the process` 或改为外层 select 不再包 goroutine。**已落地 2026-05-23**：补 godoc 锚点说明三条路径（clean exit / deadline+osExit / 测试替换 osExit）的 goroutine 生命周期，并锁定 osExit 包级 var 的设计。
...
- [x] **R234-GO-10 — runstore.trimJobLocked sort 用 cmp.Compare(UnixNano) 而非 time.Compare（P3）**：边界精度 + 风格。建议 `slices.SortFunc(items, func(a,b) int { return b.mtime.Compare(a.mtime) })`。 — 已修复（runstore.go trimJobLocked 改用 b.mtime.Compare(a.mtime)，runID tie-break 仍用 cmp.Compare 字符串），本批 PR
...
- [x] **R234-GO-12 — Scheduler 锁层级文档（s.mu vs runStore.jobLock）未注释（P3）**：建议 Scheduler 结构体注释加 `s.mu > runStore.jobLock(jobID) > recentCacheEntry.mu`。 — 已修复（scheduler.go Scheduler struct 顶部加完整 4-level 层级 godoc：s.mu > storeMu > jobLock > entry.mu，含 cacheGet 释放-重取模式指引），本批 PR
...
- [x] **R234-PERF-16 — protocol_claude.extractAskQuestion 每 assistant 事件全 block 扫描（P3）**：建议 `strings.Contains(rawContent, "AskUserQuestion")` 早 short circuit。 — 已修复 2026-05-23：ReadEvent 在调 extractAskQuestion 前先 `strings.Contains(line, askUserQuestionToolName)` 短路；同时把硬编码 "AskUserQuestion" 提为 const askUserQuestionToolName 让两处 string 保持一致。
...
- [x] **R224-GO-5 — `eventlog.invokePersistSink` `replay` 标志读取存在 sink Store/sinkReady Store 之间的 race window（P2，归档 2026-05-23）**: cli/eventlog.go:396-397 SetPersistSink 顺序已反转：先 `persistSinkPtr.Store(&p)` 再 `sinkReady.Store(true)`；godoc 显式锚定"Store the sink pointer FIRST so any concurrent Append that reads sinkReady=true will also see a valid sink"。race window 已闭合。
...
- [x] **R220-PERF-1 — `countActive()` evictOldest/Takeover/spawnSession 路径全 map scan（P1，归档 2026-05-23）**: router_lifecycle.go:944+ countActive 已被改造成"仅在 Cleanup 全量重算"，evict/takeover 路径用 `newActive` 增量+原子计数；当前注释（line 556-566）显式记录"避免 O(n) countActive"的成本权衡。
- [x] **R220-PERF-3 — `EventLog.EntriesSince` 初始 catch-up 在 RLock 下复制 500 entry × 512B（P2，归档 2026-05-23）**: cli/eventlog.go:32-41 已定义 `entriesSinceInitialCap = 16` 常量，line 1133-1139 用此常量给反向扫描的初始 slice 加 cap，避免一次性复制 500 entry。原方案"先 snapshot ring 索引"的等价效果已通过初始 cap 限流实现。
...
- [x] **R214-PERF-1 — PersistSink 单 entry 版本避免 1-slot slice alloc（归档 2026-05-23）**: cli/eventlog.go:822+ 已添加完整 archive godoc：R230-PERF-1 加了 sinkAttached 早返保护（无 sink 时跳过 slice literal），剩余 1-slot slice 在 sink-attached 路径上是 PersistSink retention contract 的"契约性 alloc"——sync.Pool/`[1]EventEntry`+`s[:]` 都会 escape 或换 Get/Put 开销，结构上无法消除。设计决策已锚定。
