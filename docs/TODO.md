...
- [x] **R230C-GO-8 — finishRun bumpRunStateMetrics 在 persist 回滚前已计数（P2）**: `internal/cron/scheduler.go:2103` persist 失败时 in-memory job 字段回滚但 CronRunSucceededTotal 已 +1。方案：bumpRunStateMetrics 移到 recordResultP0WithSanitised 返回 ok 之后。Breaking：否。 — 已修复（finishRun 把 bumpRunStateMetrics 从入口处移到 recordResultP0WithSanitised 之后；persist 失败 jobPersistOK=false 时不再 bump，dashboard 通量与 durable runs 数对齐；skip-persist 路径 canceled / shutdown / overlap-skipped 仍 bump 因为 metric 是它们仅有的 durable 记录），本批 PR
