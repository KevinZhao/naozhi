...
- [x] **R230-ARCH-7 — 错误→用户消息映射有 3 处偏序（P2）**: `usermsg.ForSendError` 是规范源，但 dispatch.sendAndReply 仍内联 ErrMaxProcs / ErrMaxExemptSessions / 超时分支，apierr.localizeAPIError 是第四套。方案：超时参数注入 usermsg 或 dispatch 侧 helper，dispatch 内联 switch 收敛。Breaking：否。 — 已修复（抽 `Dispatcher.localizedSendError(err, key)` helper，sendAndReply 的 sendFn 错误内联 switch 收敛到 helper；watchdog 计数副作用一并迁入；GetOrCreate 路径的 ErrMaxProcs/ErrMaxExemptSessions 文案与 sendFn 路径不同，保留独立分支由 R226-ARCH-12 注册式映射 RFC 跟踪），本批 PR
...
- [ ] **R230-ARCH-14 — internal/session/router_core.go 用 blank import 注册 history factory（P2）**: Sprint 1b 注释自承"将合并到 wireup 包"。任何 Router 测试都触发全局 registry 改动。方案：`internal/wireup/wireup.go` + 显式 RegisterDefaults() 在 main.go 调。Breaking：否（迁移）。2026-05-23：blank import 处加 contract 锚点，明确"导入位置导致测试隐式注册全局 factory，禁止测试假设特定 registry 形状"，等 Sprint 1b 真正迁 wireup 时补一并落地。
...
- [ ] **R226-SEC-7 — `/health` 端点无认证无限速（P3）**: 暴露 session 数 / watchdog kill 计数 / 平台名 / node 状态 / build 版本，外部 attacker 可高频枚举 infra 拓扑、估算重启时序。方案：加 per-IP rate limiter（60 req/min burst 10）。`internal/server/health.go`。 — 部分缓解 2026-05-23：R229-SEC-7 已把敏感字段下沉 auth 分支（unauth 仅 `{status, uptime}`）；剩余 unauth per-IP rate limit 未做。本批在 handleHealth 上方加 THREAT MODEL godoc 锚点：明确现状（unauth 路径常量成本、不打 log）+ 落地路径（复用 IPLimiter 仅包 unauth 分支以免影响 dashboard 1Hz polling）。
...
- [x] **R220-PERF-3 — `EventLog.EntriesSince` 初始 catch-up 在 RLock 下复制 500 entry × 512B（P2）**: 已加 godoc 锚点说明保留当前形状的理由：snapshot 后释放 RLock 再 copy 会让 Append 覆盖正在被读的 ring slot，正确性要么持锁要么 ring-wide double-buffer；hot-path k=1-5 立即 break；full-catch-up 是一次性 per-tab 不在 steady-state 路径。锚点：`internal/cli/eventlog.go::EntriesSince` 顶部。
...
- [ ] **R228-ARCH-14 — `dispatch.Dispatcher.takeoverFn`/`sendFn` closure 字段易漏 wireup（P2）**: closure-pattern 经典毛病。方案：1-method interface。Breaking：内部 wiring。涉及 `internal/dispatch/dispatch.go:82-83`。 — 部分缓解 2026-05-23：Dispatcher 上 sendFn / takeoverFn 字段加 godoc 锚点（写明哪个 production wiring 注入、为什么用 closure 而不是 interface、与 R224-ARCH-9 的关系：immediate refactor target 是减少签名里的 cli.* concrete types，而不是 function-vs-method）。仍未做：实际抽 1-method interface（要先把 cli.ImageData / cli.SendResult 从签名里挪走才有意义，由 R224-ARCH-9 跟踪）。
- [ ] **R228-ARCH-18 — `dispatch.Dispatcher.projectMgr` 仅用于 slash-command UX 但持整个 `*project.Manager`（P3）**: 30+ 方法面。方案：内部 1-method interface 注入。涉及 `internal/dispatch/dispatch.go:56`。 — 部分缓解 2026-05-23：在 dispatch/commands.go 声明 `projectManagerSurface` consumer interface（5 方法：ProjectForChat / Get / All / BindChat / UnbindAllChat）+ 加 `var _ projectManagerSurface = (*project.Manager)(nil)` 编译期 pin，配套 dispatch.go projectMgr 字段 godoc 锚点说明 R228-ARCH-18 / R218B-ARCH-2 关系。仍未做：实际把字段类型 + DispatcherConfig.ProjectMgr 切到 interface（要同步动 ~6 处测试 wire-up，留给下一次 dispatch 触动）。
...
- [x] **R230C-GO-7 — executeOpt sendCtx 用 context.Background 让 5h 任务无法 shutdown 期取消（P2）** — 评估闭合 2026-05-23（cron-fix-F2）：`internal/cron/scheduler.go:2170` R191-ARCH-M5 anchor 扩展加 R230C-GO-7 cross-ref：runDeadlineWatchdog 已覆盖 jobTimeout-fired 路径调 InterruptViaControl；stopCtx-during-long-job 走 Stop() 的 intentional-orphan path（CONTRACT block 已记录）。真要 watchdog 接 stopCtx 是另一项重构，本轮不做，文档化决策。
