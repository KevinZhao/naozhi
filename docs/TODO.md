...
- [ ] **R231-ARCH-10 — backend.Profile 注册表承诺与 wrapper.go 硬编码 switch 落差（P2）** [部分缓解 2026-05-23]: `backendDisplayName` 与 `detectCLI` 内联 switch 都已替换为 sentinel map（`displayNameByBackend` / `defaultBinaryByBackend`）+ godoc 锚点。无法直接调 `backend.LookupProfile`（cli/backend 反向 import cli 成环），sentinel 表保持 cli 为叶子。完整下沉到 `backend.Get` 需打破 import 环（cli.Protocol 类型迁出）才能做。新增 backend 时需要同步 sentinel 表 + cli/backend Profile，cli/backend test 跨检查可在 CI 暴露漏写。
...
- [ ] **R230B-ARCH-9 — Hub 持 `*cli.SubagentLinker` 内部对象指针（P1）** [接口已落地 2026-05-23]: cli 包内已新增 `cli.AgentIntrospector` 接口（Query + ProjectSessionDir）+ 编译期断言 `*SubagentLinker` 实现该接口（subagent_link.go）。Hub / wshub_agent / dashboard_agent_events 渐进迁移到 AgentIntrospector 仍待做（涉及 server 文件域，跨 lane）。新签名稳定即可让后续 server 改造收敛到这个接口。
...
- [x] **R224-GO-3 — `reconnectShims` replay 段把 `time.Now().UnixMilli()` 作为 agentToolUseMS 传入 Resolve（P1）** — 已修复 2026-05-23：reconnect replay 路径改为 `Resolve(..., 0)` 禁用时间过滤；`internal/session/router_shim.go` 调用点更新。涉及：`internal/session/router_shim.go`。
...
- [ ] **R220-PERF-3 — `EventLog.EntriesSince` 初始 catch-up 在 RLock 下复制 500 entry × 512B（P2）** [godoc 锚点 2026-05-23]: hot-path（k=1-5）已无回归——单次反向扫描 + 早退 + 懒分配 (eventlog.go EntriesSince)。剩余 worst-case 是 dashboard tab 首次订阅 afterMS=0 + 满 ring，每次开 tab 一次。完整 snapshot-outside-RLock 方案需要 head/count 各自 atomic（破坏 mu 互斥契约），单独跟踪。已加 R220-PERF-3 anchor godoc 解释当前优化与剩余边界。
