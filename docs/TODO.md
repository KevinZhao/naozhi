...
- [x] **R233-PERF-7 — agentTailer 200ms ticker buffered 超 500 时 复制 500 条（已修复 2026-05-23 复核）**: agent_tailer.go:363-382 已改 in-place copy（reuse backing array）+ 显式 zero-out tail entries 以释放底层 EventEntry 引用。R228-PERF-13 / R231-PERF-? 同根因 godoc 锚点已明示。归档关闭，本批 PR。
- [~] **R233-PERF-8 — sysession.Manager hookMu RWMutex 改 atomic.Pointer 收益评估（归档 2026-05-23）**: TODO 自身已写明 "边际收益小; RWMutex Go 1.21+ 已无锁化"。每 tick 两次 RLock 在 sysession 低频 tick 路径（非 message hot path）属于已优化区间。改 atomic.Pointer 需要 SetCallbacks 双 atomic pair，两个 hook 字段间状态可能微观失同步。归档关闭，本批 PR。
...
- [x] **R233B-PERF-6 — eventlog onAgentTaskDone 回调用 Mutex（已修复 2026-05-23 复核确认）**: 已是 atomic.Pointer[func(taskID, status string)] 模式（cli/eventlog.go:293），Append 路径无锁 Load。归档关闭，本批 PR。
...
- [x] **R233B-CR-7 — skipAppendTrim 三条件无单测（已修复 2026-05-23 复核确认）**: runstore_test.go:633-755 已含 dedicated table-driven 测试覆盖 happy path + 三个边界条件 + 缓存 miss case。归档关闭，本批 PR。
...
- [x] **R232-ARCH-2 — recordResult 与 recordResultP0WithSanitised 双轨依然存在（P1）**: 已删除 recordResult；persist_failure_test.go 两测试改调 recordResultP0WithSanitised（含 ErrClass + RunState 参数）。同根因 R230B-SEC-1 / R230C-CR-1 / R233B-CR-1 一并关闭。本批 PR（real-fix-batch-4）。
...
- [x] **R232-PERF-2 — agent_tailer.pollOnce 每事件 × 每 client 重复 marshal（已修复 2026-05-23 复核确认）**: agent_tailer.go:402-475 已实现 fan-out 优化：当 len(subs)>1 时每事件仅 marshalPooled 一次，所有 subscribers 用 SendRaw 共享 bytes；len(subs)==1 走 SendJSON 短路。R231-PERF-5 / R233-PERF-* 同根因 godoc 锚点已明示。归档关闭，本批 PR。
...
- [x] **R231-PERF-5 — agent_tailer pollOnce 每事件逐 SendJSON Marshal（已修复 2026-05-23 复核）**: 同 R232-PERF-2 落地，agent_tailer.go:402-475 fan-out marshalPooled 优化。本批 PR。
...
- [~] **R230-ARCH-15 — internal/server 90+ Go 文件单包（归档 2026-05-23 等专题 RFC）**: 拆 server 子包是 server-split-design.md / R231-ARCH-5 / R229-ARCH-1 主条目跟踪范围。R230C-ARCH-1 在 DESIGN.md 已加现状对照段。本条目作为重复登记归档关闭，本批 PR。
...
- [~] **R230-ARCH-17 — internal/cli 65+ 文件混 protocol/process/eventlog（归档 2026-05-23 等专题 RFC）**: 与 R229-ARCH-2（cli 越界）同根因、与 R230B-ARCH-11（cli 7 子领域）同根因，需要 cli-split RFC 设计完整 import graph 重排。本条目作为重复登记归档关闭，本批 PR。
...
- [x] **R230B-SEC-1 — `recordResult` 死代码缺 RunCounters 更新（P2）**: 已删除 recordResult（~85 行），测试改调 recordResultP0WithSanitised；本批 PR（real-fix-batch-4）。
...
- [~] **R230B-GO-3 — `recordResultP0WithSanitised` / `recordResult` mu Unlock 非 deferred（归档 2026-05-23）**: recordResult 已删除（real-fix-batch-4），仅余 recordResultP0WithSanitised 单条路径，两个 mu.Unlock() 调用紧贴 return（success + perr 失败）共两个 early-return，模式可控；分阶段 stateMutate/stateCommit 是更大的 refactor，且 perr 分支 fields 回滚必须在 Unlock 之前完成（否则中间状态被并发读看到）。归档关闭，本批 PR。
- [~] **R230B-GO-4 — `Hub.handleSubscribe` O(N) maxSubscribersPerKey 扫描在写锁下（归档 2026-05-23）**: 复核 wshub.go:614-637 现状：扫描已包含 `count >= maxSubscribersPerKey { break }` 短路（早返），最坏 20 次 map lookup；handleSubscribe 是低频路径（每用户每打开一个 session 一次），与 BroadcastSessionsUpdate 1Hz 的频率比可忽略。R226-SEC-8 godoc 已明示 "O(connections) scan is bounded by maxWSConns=500 and only runs on subscribe"。归档关闭，本批 PR。
...
- [~] **R230B-PERF-5 — `subagent_transcript.readLocked` 每次 open+seek+ReadAll（归档 2026-05-23 待 inotify RFC）**: 持久 fd 方案需要处理 file truncation / inode swap / log rotation 等 4 种边缘场景；R231-PERF-3 / R232-PERF-3 已通过 readBuf 复用消除 ReadAll growth-doubling allocs（最大单次开销）。50 syscall/s × 5 段（open+seek+ReadAll+close+stat）= 250 syscall/s 在 epoll_pwait 主循环里属于低噪声。等 inotify 方案 RFC（与 R230B-PERF-x 同批 RFC）。归档关闭，本批 PR。
- [~] **R230B-PERF-6 — `eventlog_bridge` 单条快路径仍 copy raw bytes（归档 2026-05-23）**: 复核 eventlog_bridge.go:99-103 + persister.SinkFor 契约：bridge 用 sync.Pool 借出 bridgeEncBuf，把 buf.Bytes() copy 出来后才能 Put 回池——否则下一个 Append 拿到 buf 时 persisterSink 仍引用 raw 切片，触发数据竞争 / 脏数据。Persister 文档亦明示"sink 可保留 entries"。方案描述违反契约。归档关闭，本批 PR。
...
- [~] **R230B-PERF-8 — `notifySubscribers` map iteration vs slice（归档 2026-05-23）**: subCount.Load() 已是 lock-free 早返路径（count==0）；count==1 fast path 引入 singleSub atomic.Pointer 需要 Subscribe/Unsubscribe 协同更新，且 map iteration over single entry 实测开销 < 50ns（map header lookup + range loop 立即退出），与 RLock+RUnlock atomic 比可忽略。优化收益不足以匹配维护成本。归档关闭，本批 PR。
...
- [~] **R229-CR-1 — cron.executeOpt 329 行高复杂度（归档 2026-05-23 等专题 RFC）**: 与 R232-ARCH-12（executeOpt 抽 step interface）同根因。R230C-CR-7（classifyExecError）已部分修复错误分类散布；preflight / send / finish 三段已分别有命名 helpers（freshContextPreflightP0 / finishRun / classifyExecError），剩下的 CAS gate / inflight metadata / metrics 三段紧密耦合 inflight 局部变量，机械抽取会引入大量参数透传。等 step interface RFC。归档关闭，本批 PR。
- [~] **R229-CR-2 — NewRouter 359 行未被 router-split 重构覆盖（归档 2026-05-23 等专题 RFC）**: 与 R231-ARCH-11（NewRouter 构造期副作用）+ R231-ARCH-4（Router god-object）同根因。NewRouter 拆 helper 需要解决三个共享状态：(1) r.mu 跨段；(2) historyWg / historyCtx 跨段；(3) wrapper 解析 + persister wiring 必须早于 store load。等 router-split-v2 RFC 落地后机械拆分。归档关闭，本批 PR。
- [~] **R229-CR-3 — reconnectShims 350 行 + 单分支 90 行（归档 2026-05-23 等专题 RFC）**: 90 行单分支密集捕获多个上下文变量（recWrapper, recBackendID, sess, sessPrevIDs, parentCtx, state, lastSeq, ids, replays, linker, ev, taskID, toolUseID 等）；processDiscoveredShim 函数签名需要 ~12 个参数才能保持原语义，反而降低可读性。R229-ARCH-2（cli 越界）/ R230-ARCH-3（接口窄化）落地后是更自然的拆分点。归档关闭，本批 PR。
- [x] **R229-CR-7 — dispatch.go 1281 行 replyTracker 与 Dispatcher 同居（P3）**: 已抽 dispatch/reply_tracker.go（446 行：type + 11 method），dispatch.go 从 1256 行降至 808 行；包内可见性不变（type/methods 仍 private），无外部 API 影响。本批 PR。
...
- [~] **R222-CR-1 — `session.NewRouter` 350 行 6+ 阶段直列（归档 2026-05-23 等专题 RFC）**: NewRouter 拆 helper 与 R229-CR-2 / R231-ARCH-11 同根因——共享 r.mu / historyWg / historyCtx / wrapper 解析跨段，机械抽取需要透传 12+ 参数反降低可读性。等 router-split-v2 RFC。本批 PR。
...
- [x] **R217-SEC-8 — `/health` cleartext 嗅探 warning（已修复 2026-05-23 复核确认）**: server.go:853 isPlaintextPublicAddr() 启动时已对所有 cleartext+token 路径发 slog.Warn（含 bearer + cookie 嗅探警示）；shouldWarnNoTokenOpen 处理 no-token 公网部署。/health 是子路径，已被通用警告覆盖。归档关闭，本批 PR。
...
- [x] **R217-CR-3 — `Cleanup` 三阶段加锁窗口**: stuckKill 路径在 Kill 前 RLock 复检 `r.sessions[key].proc == e.proc`，避免误杀已被 ResetAndRecreate / spawnSession 替换的新进程。本批 PR。
...
- [x] **R215-SEC-P3-1 — Shim `auth_token` 明文写 state 文件（已修复 2026-05-23 复核确认）**: shim/state.go:61 WriteStateFile 已显式 0600 (godoc 自承"atomically writes the state to path with mode 0600")，state_test.go 多处契约测试。同 UID 进程读取仍是 OS 信任边界，运维应配合 systemd User= + 0600 防御。本批 PR。
...
- [x] **R227-GO-4 — `Router.Shutdown` test fallback `time.Sleep(100ms)` busy-poll（P2）**: shutdownCond nil 分支已加 slog.Warn 提示测试作者绕过 NewRouter；godoc 说明 fallback 仅在 `&Router{}` 裸构造测试出现，生产路径走 cond.Wait。本批 PR。
...
- [~] **R227-ARCH-15 — `eventlog_bridge` 在 cli↔persist 中介中做 EventEntry→Entry marshal（归档 2026-05-23 等专题 RFC）**: bridge 当前用 sync.Pool 借出 bridgeEncBuf 生产 raw bytes 再 copy 出来——必须 copy 才能 Put 回池（与 R230B-PERF-6 同根因，sink 可保留 slice 引用）。把 marshal 上移到 EventEntry 上能减少一层调用，但池/契约管理仍要在某层完成。机械抽方法解决不了责任划分。归档关闭，本批 PR。
...
- [x] **R230C-PERF-1 — connector_subscribe 每次 notify 都 Snapshot()（P1）**: ManagedSession 新增轻量 State() string + DeathReason() string 方法，connector_subscribe 用 sess.State() / sess.DeathReason() 替代 Snapshot()，避免 dashboard 池化的 mirror SetModel 副作用 + 10+ atomic.Loads + slice 拷贝。R230C-PERF-10（双取 eventlog.mu）同根因一并解决。本批 PR。
- [~] **R230C-PERF-2 — eventlog.Append 单条路径每次分配 `[]EventEntry{e}`（归档 2026-05-23 误报）**: 复核 PersistSink 契约：persister.SinkFor 把 entries slice 引用塞进 batchJob 经 channel 异步处理；EventLog 持久缓冲会被下一次 Append 覆盖前 persister 还在用，导致数据竞争 / 脏写。方案描述违反"sink 可保留 slice"契约。R219-PERF-4 已经做过 [1]EventEntry 栈尝试并明确证伪。归档关闭，本批 PR。
...
- [~] **R230C-PERF-6 — completeSubscribe 调两次 Snapshot()（归档 2026-05-23 误报）**: 复核 wshub.go:687/754 现状：两个 Snapshot 调用分别在不同分支——line 687 在 !sess.HasProcess() 的 suspended/resuming 分支；line 754 在 process-attached 正常路径。两条路径不同时执行（前者 return 提前退出），无法共用 snap。归档关闭，本批 PR。
...
- [~] **R230C-PERF-8 — resubscribeEvents 每轮 h.mu.RLock + map read 检查 subGen（归档 2026-05-23 误报）**: 复核现状：gen 参数是"resubscribe 启动时"的快照值；检查目的是"有没有更新的 subscribe 抢占同 key"——必须读 c.subGen[key] 当前值与 gen 比较，否则永远不会感知 takeover（R163 contract）。改 atomic.Uint64 是正路但跨包接口变化大；当前 RLock 12-iter × 5s 间隔实际频率极低，与 fast path 无关。归档关闭，本批 PR。
- [x] **R230C-PERF-10 — connector_subscribe Snapshot+EventEntriesSince 双取 eventlog.mu（P3）**: 同 R230C-PERF-1 落地——sess.State() 不取 eventlog.mu，仅一个 atomic.Load；EventEntriesSince 仍是另一锁但是必须的（读事件必须持锁）。本批 PR。
...
- [x] **R230C-CR-7 — executeOpt 错误分类逻辑散在 4 个 finishRun 分支（P2）**: 已抽 `classifyExecError(err, defaultErrClass) (state, errClass, canceled)` helper，GetOrCreate / Send 两个 err 分支收敛到一行调用，自然区分 ErrClassSessionError / ErrClassSendError。本批 PR。
...
- [x] **R230C-ARCH-1 — DESIGN.md 第 5 节 HTTP Server 描述与现实严重失真（P1）**: 已在 DESIGN.md §5 后追加"实际 vs 理想"段，明确 server 现状（90+ 文件 / 60+ 路由 / 17+ handler 子结构体），并链接 server-split-design.md / R231-ARCH-5 / R229-ARCH-1 拆包 RFC。本批 PR。
...
- [x] **R230C-ARCH-17 — DESIGN.md 缺 Backend Extension Points 节（P3）**: 已在 DESIGN.md 添加 "Backend Extension Points" 节列 4 接口（backend.Profile 必做 / cli.Protocol 必做 / history.Source 可选 / shim hint 可选），含 claude+kiro 参考实现指引。本批 PR。
