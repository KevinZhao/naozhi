...
- [ ] **R248-GO-1 — closureCapabilities.Send c.send==nil 运行时 panic（P3）** [REFACTOR]: `internal/dispatch/capabilities.go:104-109` 当 NewDispatcher 走 closure 适配器分支但 SendFn 为 nil（用户只设了 TakeoverFn/ReplyFooterFn），首次 Send 调用才 panic。契约违规应在构造期暴露。方案：dispatch.go:286-297 的 closure 分支检测 `cfg.SendFn == nil` 时直接 panic("dispatch: SendFn required when other *Fn set")，与 NoopCapabilities 的 boot-time fail-loud 语义一致；或改 closureCapabilities.Send 在 nil 时返 error（破坏现有 panic 契约，需 RFC）。优先级低因生产 wireup 走 Capabilities 直注入路径，仅测试 fixture 可能踩到。
- [ ] **R248-GO-2 — NewDispatcher Capabilities 与 *Fn 同时设置静默忽略 *Fn（P3）** [REFACTOR]: `internal/dispatch/dispatch.go:286-297` `caps := cfg.Capabilities; if caps == nil { ... }` —— 若调用者同时设置 `Capabilities` 和 `SendFn`（迁移期常见误用），SendFn 静默丢失，hot-path 走 Capabilities.Send。方案：`if cfg.Capabilities != nil && (cfg.SendFn != nil || cfg.TakeoverFn != nil || cfg.ReplyFooterFn != nil) { slog.Warn("dispatch: Capabilities set; legacy *Fn fields ignored") }` 或构造期 panic。一次性日志，无 hot-path 成本。
...
- [ ] **R248-TEST-1 — NoopCapabilities.Send panic 契约无测试 anchor（P1）** [REFACTOR]: `internal/dispatch/capabilities.go:81` 历史 sendFn "no-fallback" 契约改成 `NoopCapabilities.Send` panic，但若有人改成 silent return 整个 IM 路径会静默丢消息，CI 不会响。方案：加 `TestNoopCapabilities_SendPanics`（defer recover + 检查 panic 文本含 "not wired"），并对 Takeover 返 false / ReplyFooter 返 "" 加表驱动断言。
- [ ] **R248-TEST-2 — DispatcherConfig.Capabilities 优先级 3 路径无测试（P1）** [REFACTOR]: `internal/dispatch/dispatch.go:286-297` 三段 if 分支（Capabilities > 三 *Fn 闭包 > Noop）无回归测。方案：加 `TestNewDispatcher_CapabilitiesPrecedence` 三 case：仅 SendFn → closureCapabilities；仅 Capabilities → 直接用；两者都设 → Capabilities 赢（断言 d.caps.Send 触发 cfg.Capabilities 而非 cfg.SendFn）。
- [ ] **R248-TEST-3 — spawningKeys chan close-then-delete 不变量缺测试（P1）** [REFACTOR]: `internal/session/router_test.go:1666 TestSpawningKeys_ObservableDuringSpawn` 只在内存 set/observe/clear marker，**不**实际触发 close→waiter 唤醒路径。方案：增 3 测试 (a) `TestSpawningKeys_FailedSpawnWakesWaiters` — N goroutine 同 key 在 spawn 失败下应 < 100ms 全返；(b) `TestSpawningKeys_CloseBeforeDelete_Order` — anchor grep router_lifecycle.go defer 内 close 行号 < delete；(c) `TestSpawningKeys_CtxCancelPriorityOverDoneCh` — ctx 先 cancel 应返 ctx.Err 而非 retry。
...
- [ ] **R248-TEST-5 — AgentLinker typed-nil 陷阱无测试（P2）** [REFACTOR]: `internal/server/dashboard_agent_events_test.go` PR #331 把字段从 *cli.SubagentLinker 改 agentlink.AgentLinker，wshub_agent.go:67-73 / dashboard_agent_events.go:86-94 都做了 concrete-layer nil check 但无测试。方案：加 `TestLinkerForSession_TypedNilGuard` mock sess.SubagentLinker() 返 typed-nil *cli.SubagentLinker，断言 linkerForSession() 返 untyped nil（== nil 为 true），HTTP 路径返 404 而非 panic。
- [ ] **R248-TEST-6 — reply_footer_test.go 用 &Dispatcher{} 直构造绕过 NewDispatcher（P2）** [REFACTOR]: `internal/dispatch/reply_footer_test.go:57` 该绕过让 caps==nil 的合法性不可见。方案：加 `TestNewDispatcher_CapsAlwaysNonNil` — DispatcherConfig{} 全空构造，断言 d.caps != nil（应为 NoopCapabilities{}）并触发 Takeover/ReplyFooter 返默认值。
- [ ] **R248-TEST-7 — counter_wiring 缺负向断言 wshub.go 不能再有 WSAuthFailTotal.Add（P2）** [REFACTOR]: `internal/metrics/counter_wiring_contract_test.go:42-50` 当前只正向断言 wshub_upgrade.go 有 ≥2 个 Add，未防止误把 fallback 加回 wshub.go（双计数）。方案：加 `TestOBS2_WSAuthFailNotInMainHub` — 扫 wshub.go 不应包含该 token。
- [ ] **R248-TEST-8 — debounce_contract_test 缺 AfterFunc 不回 wshub.go 负向断言（P3）** [REFACTOR]: `internal/server/debounce_contract_test.go:43`。方案：在测试加 `if regexp.MustCompile("time.AfterFunc(debounceInterval").Match(srcWshub) { t.Error(...) }`。
- [ ] **R248-TEST-9 — TestConcurrentGetOrCreate_SameKey_Race 只跑不断言（P3）** [REFACTOR]: `internal/session/router_test.go:1357` 10 goroutine 打同 key 但完全丢弃返回值。方案：加断言 (a) 失败原因都是 "spawn"；(b) wg.Wait 必须在 N×timeout 内完成，避免 close(doneCh) 漏一行只能 CI 超时才出错。
...
- [ ] **R248-ARCH-2 — NoopCapabilities.Send 应"构造时 panic"而非运行时（P1）** [REFACTOR]: `internal/dispatch/capabilities.go:81` godoc 自陈"missing wireup must surface as a constructor-time panic, not a silent drop"，但实现是消息抵达时 panic：boot 完成 + 健康探针 ok + 第一条消息进来才崩，systemd 立即重启进入循环 — 比 silent drop 更糟。方案：NewDispatcher 显式校验 cfg.Capabilities==nil && cfg.SendFn==nil 时直接 panic("dispatch: SendFn or Capabilities required")。
...
- [ ] **R248-CR-1 — capabilities.go 注释/代码比 1:1（P1）** [REFACTOR]: `internal/dispatch/capabilities.go:1-91` 类型 godoc 含 60+ 行混杂 review-history 与契约说明，超过实际 type+method 代码量。方案：压成 1-2 行尾注或挪到 docs/rfc/server-split.md，保留接口契约（Send 必须、Takeover/ReplyFooter 默认）。
- [ ] **R248-CR-2 — wshub.go wiredLinkers 单字段堆叠 5 个 review 编号（P1）** [REFACTOR]: `internal/server/wshub.go:175-194` 注释提 R201/R230/R231/R233B/R239 等价于 git log 摘要。方案：留 1 个 anchor (R201-CRIT-2 - 历史 leak) + 1 行 R239-ARCH-I（解释当前 interface 化的语义等价性证明），其他可删。
