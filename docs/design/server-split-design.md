...
| 片 | 范围 | 破坏面 | 前置 |
|---|---|---|---|
| 4a | 残留 6 个 `dashboard_*.go`（ccassets / orient / planner_stats / send / system / uiprefs）按既有 Deps 模式迁入 `internal/dashboard/*`；`dashboard_send.go`（1089 行）单独一个 PR | 低（模式已验证） | 无 |
| 4b | `buildServer` 的构造互连段上移 `wireup/boot.go`：sessionGuard、msgQueue、nodeCache、scratchPool、retiredStore 的构造与注入；server 只接收成品 | 中（构造顺序敏感，需保持 StartupCtx 语义） | 4a |
